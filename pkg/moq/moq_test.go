...
// expected.
// see https://github.com/liruohrh/moq/issues/5
...
func TestImports(t *testing.T) {
	...
	strs := []string{
		`	"sync"`,
		`	"github.com/liruohrh/moq/pkg/moq/testpackages/imports/one"`,
	}
	for _, str := range strs {
		if !strings.Contains(s, str) {
			t.Errorf("expected but missing: \"%s\"", str)
...
func TestMockGolden(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			// See https://github.com/liruohrh/moq/issues/124
			name:       "SliceResult",
		...
		},
		{
			...
			// blank identifier.
			// See https://github.com/liruohrh/moq/issues/70
			name:       "BlankID",
		...
		},
		{
			...
			// See https://github.com/liruohrh/moq/issues/94
			name:       "PkgShadow",
...
func matchGoldenFile(goldenFile string, actual []byte) error {
	...
	// go test -v -run '^<Test-Name>$' github.com/liruohrh/moq/pkg/moq -update
	if *update {
...
func TestVendoredInterface(t *testing.T) {
	...
	incorrectImport := `"github.com/liruohrh/moq/pkg/moq/testpackages/vendoring/vendor/github.com/sudo-suhas/moq-test-pkgs/somerepo"`
...
// TestDotImports tests for https://github.com/liruohrh/moq/issues/21.
...
func TestMockError(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			name:     "UnexpectedType",
			namePair: "Person",
			wantErr:  "Person (github.com/liruohrh/moq/pkg/moq/testpackages/example.Person) is not an interface",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
